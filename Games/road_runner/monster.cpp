...
bitmap bitmap_mons(character_mons kind_mons)
{
    ...
    case FIRE:
        return bitmap_named("fire_bmp");
    case ROCK:
        return bitmap_named("rock_bmp");
    case FGround1:
...
mons_data monster()
{
    ...
    bitmap default_bitmap = bitmap_mons(MONSTER);
        ...
        sprite_add_layer(result.mons_sprite, bitmap_mons(FIRE), "fire_bmp");
        sprite_add_layer(result.mons_sprite, bitmap_mons(ROCK), "rock_bmp");
...
{
    character_mons cmons = MONSTER;
    character_mons cmons1 = MONSTER1;
    if(mons.mleve == 2){

        cmons = FIRE;
        cmons1 = ROCK;
        sprite_show_layer(mons.mons_sprite, static_cast<int>(FIRE));
        sprite_hide_layer(mons.mons_sprite, static_cast<int>(MONSTER));
        sprite_hide_layer(mons.mons_sprite, static_cast<int>(MONSTER1));
    }else{
        sprite_hide_layer(mons.mons_sprite, static_cast<int>(FIRE));
        sprite_hide_layer(mons.mons_sprite, static_cast<int>(ROCK));
    }

    if (temp == 1)
    {
        sprite_show_layer(mons.mons_sprite, static_cast<int>(cmons));
        sprite_hide_layer(mons.mons_sprite, static_cast<int>(cmons1));

        move_sprite(mons.mons_sprite, {1, 0}, 5);
    }
    if (temp == 0)
    {
        sprite_show_layer(mons.mons_sprite, static_cast<int>(cmons1));
        sprite_hide_layer(mons.mons_sprite, static_cast<int>(cmons));
